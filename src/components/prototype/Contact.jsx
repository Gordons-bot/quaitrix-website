...
const RADII = {
  none: 'rounded-none', sm: 'rounded-sm', md: 'rounded-lg', lg: 'rounded-xl', xl: 'rounded-2xl'
};

export default function Contact({ section }) {
  const { variant = 'standard', props = {} } = section;
    ...
    <section id="contact" className="py-20 bg-white">
      <div className="max-w-3xl mx-auto px-6 sm:px-8">
        <div className="text-center mb-12">
          {heading?.text && <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-gray-900">{heading.text}</h2>}
        ...
        <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
          <div className="grid sm:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" className={`w-full px-4 py-3 border border-gray-300 ${r} focus:ring-2 ${p.ring} focus:border-transparent outline-none`} placeholder="Your name" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Email</label>
              <input type="email" className={`w-full px-4 py-3 border border-gray-300 ${r} focus:ring-2 ${p.ring} focus:border-transparent outline-none`} placeholder="you@example.com" />
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Message</label>
            <textarea rows={5} className={`w-full px-4 py-3 border border-gray-300 ${r} focus:ring-2 ${p.ring} focus:border-transparent outline-none resize-none`} placeholder="Tell us about your project..."></textarea>
          </div>
          <div className="text-center">
            <button type="submit" className={`inline-flex px-8 py-4 text-white ${p.bg} ${p.hover} ${r} font-semibold shadow-lg transition-all hover:scale-[1.02]`}>
              Send Message
            </button>
          </div>
        </form>
