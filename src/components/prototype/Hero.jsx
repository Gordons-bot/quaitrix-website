        ...
        <div className="max-w-7xl mx-auto px-6 sm:px-8">
          <div className="text-center max-w-3xl mx-auto">
            ...
            <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
              {ctaPrimary?.text && (
                <a href={`#${(ctaPrimary.action && ctaPrimary.action.split(':')[1]) || 'contact'}`}
              ...
              {ctaSecondary?.text && (
                <a href={`#${(ctaSecondary.action && ctaSecondary.action.split(':')[1]) || 'features'}`}
            ...
            {image?.id && (
              <div className="mt-16">
                <img src={`https://images.unsplash.com/${image.id}?w=1200&h=600&fit=crop`}
                     alt={image.alt || 'Hero'} className={`w-full ${r} shadow-xl`} loading="eager" />
              </div>
            )}
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <div className="mt-8 flex gap-4">
              {ctaPrimary?.text && (
                <a href="#contact" className={`inline-flex px-6 py-3 text-white ${p.bg} ${p.hover} ${r} font-semibold shadow-lg`}>
                  {ctaPrimary.text}
                </a>
              )}
            </div>
          ...
          {image?.id && (
            <img src={`https://images.unsplash.com/${image.id}?w=800&h=600&fit=crop`}
                 alt={image.alt || 'Hero'} className={`w-full ${r} shadow-2xl`} loading="lazy" />
          )}
        </div>
      </div>
    </section>
